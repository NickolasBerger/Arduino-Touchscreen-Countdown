...
//adjust password & output pin here
...
{
  tft.fillRect(x, y, BUTTON, BUTTON, BLACK);
}
...
void loop() {
  ...
  if (counter == 6)
  {
    ...
    if (p.y > FOURTHR && p.y < FOURTHR + BUTTON && p.x > THIRDC && p.x < THIRDC + BUTTON)
    {
      if (p.x > FIRSTC && p.x < FIRSTC + BUTTON) //back button
      {
      ...
      if (p.x > THIRDC && p.x < THIRDC + BUTTON)
      {
        digitalWrite(outputPin, LOW);
        resetGuess();
        counter = 0;
